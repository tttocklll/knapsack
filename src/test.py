...
def main():
    ...
    with open(f"../log/log_{now}.csv", "a") as log:
        log.write("problem, best, worst, average, std\n")
    for i in range(1, 9):
        problem = f"p0{i}"
        # read problem files
        ...
        with open(f"../log/log_{now}.csv", "a") as log:
            log.write(f"{problem}, {max(result_bfwa)}, {min(result_bfwa)}, {mean(result_bfwa)}, {stdev(result_bfwa)}\n")
        for _ in range(n):
            cur_sfla = MDSFLA(capacity, weight, value)
            result = cur_sfla.solve()
            result_sfla.append(result.fitness)
        with open(f"../log/log_{now}.csv", "a") as log:
            log.write(f"{problem}, {max(result_sfla)}, {min(result_sfla)}, {mean(result_sfla)}, {stdev(result_sfla)}\n")
